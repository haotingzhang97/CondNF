...
def ged_mnist(data_x, target_x, p, model, opt, n_samples = 10):
    ...
    for digit in range(10):
        ...
        i = ind[np.random.randint(0, len(ind), 10)]
        ...
        dSS = 2 * p[digit, 0] * p[digit, 1] * np_L1loss(data_gt_red, data_gt_green) + \
        ...
        if opt.model_name == 'pix2pix':
            output_mat = np.zeros((n_samples,len(i),3,dim_x,dim_x))
            ...
            for k1 in range(n_samples):
                for k2 in range(n_samples):
                    #dYY += np.mean(np.absolute(output_mat[k1, :, :, :, :] - output_mat[k2, :, :, :, :]))
                    dYY += np_L1loss(output_mat[k1, :, :, :, :], output[k2, :, :, :, :])
            dYY /= (n_samples * n_samples)

        if opt.model_name == 'MSGAN':
            output_mat = np.zeros((n_samples,len(i),3,dim_x,dim_x))
            for ij in i:
                input = data_x[ij, :, :, :].to(device).float()
                model.isTrain = False
                model.set_input(input)
                nz = 8
                z_samples = model.get_z_random(1, nz)
                #for nn in range(n_samples):
                    #_, fake_B, _ = model.test(z_samples[[nn]])
                _, fake_B, _ = model.test(z_samples[[0]])
                if device == 'cuda':
            ...
            for k1 in range(n_samples):
                for k2 in range(n_samples):
                    dYY += np_L1loss(output_mat[i, :, :, :, :], output[j, :, :, :, :])
            dYY /= (n_samples * n_samples)
